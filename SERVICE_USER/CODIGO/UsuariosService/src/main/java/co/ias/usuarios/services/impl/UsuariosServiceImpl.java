...
import static co.ias.usuarios.utils.AppConstants.NO_USER_IN_SESSION_CONSTANT;
...
import co.ias.common.response.LoginResponse;
...
import co.ias.usuarios.repositories.PasswordHistorialOwnRepository;
...
public class UsuariosServiceImpl implements CerberoService {
			...
			if (usuario != null) {
				...
				if(validPasswordReset(usuario)) {
							...
							if (ipInWhiteList(clientIp, usuario.getListIpPermitidas()) && validateIPActive
								...
								if (oauthResponse != null && oauthResponse.getAccessToken() != null
									...
									logger.info(mrc.getMessage(
												"ias.security.autenticacion.tokenclient.crear.success", null,
												null));
									...
									logger.info("User: " + authwrapper.getUser() + " " + mrc
											.getMessage("ias.security.autenticacion.usuario.login.success", null, null));
									...
									if (oauthResponse != null && oauthResponse.getMessage() != null) {
										logger.info(mrc.getMessage("ias.security.autenticacion.token.error", null, null)
												+ " : " + oauthResponse.getMessage());
										if (oauthResponse.getCode() == org.apache.http.HttpStatus.SC_UNAUTHORIZED) {
											errorMessage = mrc.getMessage(
													"ias.security.autenticacion.tokenclient.unauthorized.error", null,
													null);
										}
									}
									else {
										logger.info(
											mrc.getMessage("ias.security.autenticacion.token.error", null, null) + " : "
														+ mrc.getMessage(
																"ias.security.autenticacion.tokenclient.timeout.error",
																null, null));
										errorMessage = mrc.getMessage(
												"ias.security.autenticacion.tokenclient.timeout.error", null, null);
									}
									messageResponse.setMessage(mrc.getMessage("ias.security.autenticacion.token.error",
								...
								auditFailLogin(authwrapper,
										mrc.getMessage("ias.security.autenticacion.usuario.IP.error", null, null), 
													   clientIp);
							}
						}
						else {
							if (MasterSession.instance().containsKey(authwrapper.getUser(),"sessionCounter")) {
								if (!validateAllowedAttempts(authwrapper.getUser())) {
									...
									messageResponse.setMessage(
										mrc.getMessage("ias.security.autenticacion.usuario.login.error", null, null));
									logger.info(
										mrc.getMessage("ias.security.autenticacion.usuario.login.error", null, null));
									...
									auditFailLogin(authwrapper,
											mrc.getMessage("ias.security.autenticacion.usuario.login.error", null,
															null), clientIp);
								}
							}
					...
					if (usuario.getEstado().equalsIgnoreCase(EstadosUsuarioEnum.BLOQUEADO.name())) {
						messageResponse.setMessage(
						mrc.getMessage("ias.security.autenticacion.usuario.bloqueado.error", null, null));
						logger.error(mrc.getMessage("ias.security.autenticacion.usuario.bloqueado.error", null, null));
					}
					else {
						messageResponse.setMessage(
							mrc.getMessage("ias.security.autenticacion.usuario.inactivo.error", null, null));
						auditFailLogin(authwrapper,
							mrc.getMessage("ias.security.autenticacion.usuario.inactivo.error", null, null), clientIp);
						logger.error(mrc.getMessage("ias.security.autenticacion.usuario.inactivo.error", null, null));
					}
				...
				messageResponse.setMessage(
					mrc.getMessage("ias.security.autenticacion.usuario.resetPasswordExpire", null, null));
					logger.error(mrc.getMessage(
								 "ias.security.autenticacion.usuario.resetPasswordExpire", null, null));
			}
		}
		else {
			messageResponse.setMessage(mrc.getMessage("ias.security.autenticacion.usuario.inactivo.error", null, null));
			logger.error(mrc.getMessage("ias.security.autenticacion.usuario.inactivo.error", null, null));
	...
	public Response logout(HttpServletRequest request) {
		...
		try {
			...
			logger.info(mrc.getMessage("ias.security.autenticacion.usuario.logout.validacion.success", null, null));
			...
			logger.info(mrc.getMessage("ias.security.autenticacion.usuario.logout.borrar.session.success", null, null));

			messageResponse.setMessage(mrc.getMessage("ias.security.autenticacion.usuario.logout.success", null, null));
			logger.info("User: " + session.getUsuario().getLogin() + " "
					+ mrc.getMessage("ias.security.autenticacion.usuario.logout.success", null, null));
	...
	private void auditLogin(Session session) {
		...
		auditAuth.setDescripcion(mrc.getMessage("ias.security.autenticacion.usuario.login.success", null, null));
	...
	private LoginResponse login(OAuthResponse token, Usuario usuario, String clientIp,
		...
		loginResponse.setUriGuardarClave(AppPaths.IAS_GENERAL + AppPaths.IAS_GENERAL_ACCESS_UPDATE_PASSWORD);
		loginResponse.setUriValidarClaveHistorico(
				AppPaths.IAS_GENERAL + AppPaths.IAS_GENERAL_ACCESS_VALIDATE_PASSWORD_HISTORICO);
		loginResponse.setUriValidarClaveRestringido(
				AppPaths.IAS_GENERAL + AppPaths.IAS_GENERAL_ACCESS_VALIDATE_PASSWORD_DICCIONARIO);
		loginResponse.setUriValidarUsuarioDocumentoEntidad(
				AppPaths.IAS_GENERAL + AppPaths.GENERAL_ACCESS_VALIDATE_USER_ENTITY_ID_CONTEXT);
		loginResponse.setUriValidarPerfilEliminar(
				AppPaths.IAS_GENERAL + AppPaths.VALIDATE_PROFILE_FOR_DELETE_CONTEXT);
		loginResponse.setUriLogout("/authenticate" + AppPaths.Security.IAS_SECURITY_LOGOUT_PATH);
		loginResponse.setUriValidaToken(AppPaths.IAS_GENERAL_ACCESS_VALIDATE_TOKEN);
		//loginResponse.setUriOperaciones(AppPaths.Operacion.PATH);
		loginResponse.setUriValidarFecha(AppPaths.IAS_GENERAL + AppPaths.IAS_GENERAL_ACCESS_VALIDATE_DATE);

		// Establece la respuesta
		loginResponse.setMessage(mrc.getMessage("ias.security.autorizacion.resourceAccess.OK", null, null));
...
//		}
	...
	private boolean validateAllowedAttempts(String user) {
		int quantityAttemps = AppConfiguration.intance().getConfiguracionIASMap()
							  .get(AppConstants.General.IAS_APPLICATION_CONFIGURATION)
		...
		if (usuarioAttemp != null && usuarioAttemp.getEstado().equalsIgnoreCase(EstadosUsuarioEnum.ACTIVO.name())) {
			...
			case AppConstants.General.BLOQUEO_INTENTOS_PERMITIDOS:
				logger.info("Usuario: " + authwrapper.getUser() + " "
						+ mrc.getMessage("ias.security.autenticacion.usuario.bloqueo.intentos.error", null, null));
				messageResponse.setMessage(
						mrc.getMessage("ias.security.autenticacion.usuario.bloqueo.intentos.error", null, null));
				break;
			case AppConstants.General.BLOQUEO_IP_PERMITIDA:
				logger.info("Usuario: " + authwrapper.getUser() + " "
						+ mrc.getMessage("ias.security.autenticacion.usuario.bloqueo.ip.motivo.error", null, null));
				messageResponse.setMessage(
						mrc.getMessage("ias.security.autenticacion.usuario.bloqueo.ip.motivo.error", null, null));
				break;
			default:
				logger.info("Usuario: " + authwrapper.getUser()
						+ mrc.getMessage("ias.security.autenticacion.usuario.login.error", null, null));
				messageResponse
						.setMessage(mrc.getMessage("ias.security.autenticacion.usuario.login.error", null, null));
				break;
			}

		}
		else
			messageResponse.setMessage(mrc.getMessage("ias.security.autenticacion.usuario.login.error", null, null));
		...
		if (wrapper != null
			...
			if (usersListSize > 0 && !sameUser) {
				...
				if(usersListSize == 1){
					...
					otrasOpciones.setBusinessErrorCode(AppConstants.IAS_USER_ROL_ENTIDAD_ONE);
				...
				resourceResponse.setMessage(mrc.getMessage(
										"ias.security.autorizacion.resourceAccess.OPTIONS.success",
										null, null));
				logger.info(mrc.getMessage("ias.security.autorizacion.resourceAccess.OPTIONS.success",
								null, null));
				...
				if (usuariosExistentes.size() < 2) {
					...
					if (usuariosExistentes.size() == 0) {
						...
						if (existentUser.getUsuarioRolEntidad().getTipoRol() 
							...
							if(caso == 0){
								otrasOpciones.setBusinessErrorCode(AppConstants.IAS_USER_ROL_ENTIDAD_TWO);
								resourceResponse.setOtrasOpciones(otrasOpciones);
								resourceResponse.setCode(org.apache.http.HttpStatus.SC_OK);
				...
				resourceResponse.setCode(org.apache.http.HttpStatus.SC_OK);
				resourceResponse.setMessage(mrc.getMessage(
										"ias.security.autorizacion.resourceAccess.POST.success",
										null, null));
			
				logger.info(mrc.getMessage("ias.security.autorizacion.resourceAccess.POST.success",
								null, null));
			}
		}
		else {

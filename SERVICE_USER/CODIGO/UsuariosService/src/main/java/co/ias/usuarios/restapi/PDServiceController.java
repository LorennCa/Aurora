...
public class PDServiceController {
	...
	public ResponseEntity<?> validateClave(HttpServletRequest request, @RequestBody ClaveWrapper clave) throws Exception {
		...
		try {
			...
			resourceResponse.setResource(AppPaths.Dictionary.IAS_USER_PASSWORD_COMPLETE_CONTEXT);
			...
			logger.error(mrc.getMessage("ias.recurso.PUT.ERROR", null, null),
		...
		try {
			...
			if (null != pass.getClave() && !pass.getClave().isEmpty()) {
				...
				if (pdservice.inRestrictedWords(pass.getClave())) {
					...
					response.setMessage(mrc.getMessage("ias.security.autorizacion.resourceAccess.OK", null, null));
					logger.info(mrc.getMessage("ias.security.autorizacion.resourceAccess.OK", null, null));
				}
				else {
					response.setCode(org.apache.http.HttpStatus.SC_NO_CONTENT);
					response.setMessage(mrc.getMessage("ias.security.autorizacion.resourceAccess.NOT_FOUND",
															   null, null));
					logger.info(mrc.getMessage("ias.security.autorizacion.resourceAccess.NOT_FOUND", null, null));
				...
				response.setMessage(mrc.getMessage("ias.recurso.params.error", null, null));
				logger.info(mrc.getMessage("ias.recurso.params.error", null, null));
			...
			logger.error(mrc.getMessage("ias.recurso.PUT.ERROR", null, null),
					"Diccionario Claves", " ", " ", e.getMessage());
			response.setMessage(mrc.getMessage("ias.recurso.PUT.ERROR", null, null));
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
		}
		return ResponseEntity.status(HttpStatus.OK).body(response);
	}
	
	@ApiOperation(value = AppPaths.Dictionary.IAS_DICCIONARIO_CLAVES, nickname = "Diccionario", notes = "Trae las palabras restringidas para claves")
	@ApiImplicitParams(value = { @ApiImplicitParam(name = "resourceAccessWrapper", value = "JSON ResourceAccessWrapper", required = true, dataType = MediaType.APPLICATION_JSON_VALUE) })
	...
	public ResponseEntity<?> get(HttpServletRequest request) throws Exception {
		...
		try {
			...
			resourceResponse.setCode(org.apache.http.HttpStatus.SC_NO_CONTENT);
			resourceResponse.setMessage(mrc.getMessage("ias.security.autorizacion.resourceAccess.NOT_FOUND", null,
													   null));
			// envia nuevamente el accessToken y uri
			resourceResponse.setAccessToken(request.getParameter("accesstoken").toString());
			resourceResponse.setResource(AppPaths.Dictionary.IAS_DICCIONARIO_CLAVES);
	...
	@ApiOperation(value = AppPaths.Dictionary.IAS_DICCIONARIO_CLAVES, nickname = "Diccionario", notes = "Actualiza la cadena de palabras restringidas para claves")
	@ApiImplicitParams(value = { @ApiImplicitParam(name = "resourceAccessWrapper", value = "JSON ResourceAccessWrapper", required = true, dataType = MediaType.APPLICATION_JSON_VALUE) })
	...
	public ResponseEntity<?> update(HttpServletRequest request, @RequestBody ClaveWrapper cadenaClaves)
		...
		try {
			...
			if (null != diccionario) {
				// Set the Response
				logger.info(mrc.getMessage("ias.recurso.PUT", null, null), "Diccionario de Claves", null, null);
				...
				resourceResponse.setOtrasOpciones(otrasOpciones);
				resourceResponse.setCode(org.apache.http.HttpStatus.SC_OK);
				resourceResponse.setMessage(mrc.getMessage("ias.security.autorizacion.resourceAccess.PUT.success",
															null, null));
			}
		}
		catch (Exception e) {
			String errorMessage = mrc.getMessage("ias.recurso.PUT.ERROR", null,null);
			logger.error(errorMessage, e.getMessage());
			resourceResponse.setCode(org.apache.http.HttpStatus.SC_INTERNAL_SERVER_ERROR);
			resourceResponse.setMessage(errorMessage+"\n"+e.getMessage());
			return ResponseEntity.status(HttpStatus.NO_CONTENT).body(resourceResponse);

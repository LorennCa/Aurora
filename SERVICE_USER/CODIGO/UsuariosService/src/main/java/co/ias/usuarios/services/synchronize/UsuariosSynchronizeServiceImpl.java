...
import co.ias.usuarios.restapi.UserController;
	...
	public synchronized Usuario saveUserNew(Usuario usuario) throws Exception {
		...
		if (resourceResponse.getCode().equals(HttpStatus.SC_NO_CONTENT)) {
			...
			try {
				...
				throw new Exception(mrc.getMessage("ias.recurso.POST.ERROR", null, null));

			}

		} else {
			throw new Exception(mrc.getMessage("ias.usuario.business.validation.rolEntidad.ERROR", null, null));
		}
		return usuario;

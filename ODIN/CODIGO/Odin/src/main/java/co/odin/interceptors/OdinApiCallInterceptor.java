...
@Component
public class OdinApiCallInterceptor extends HandlerInterceptorAdapter {

	/**
	 * Logger de apache log4j
	 */
	private static final Logger logger = LogManager.getLogger(OdinApiCallInterceptor.class);
	...
	public boolean preHandle(HttpServletRequest request,
		...
		try {
			...
			if (scope !=null && !scope.isEmpty() && accessToken != null && !accessToken.isEmpty() 
				...
				messageReponse.setMessage(mrc.getMessage("odin.security.autorizacion.resourceAccess.error", null,
														 null));
			...
			new Exception(e);
			logger.info(mrc.getMessage("odin.security.autorizacion.resourceAccess.interceptor.error", null, null),
									   e.getMessage());

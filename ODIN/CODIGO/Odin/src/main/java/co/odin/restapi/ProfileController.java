...
import co.common.response.PerfilResponse;
...
@RestController
@RequestMapping(AppPaths.Perfil.ODIN_PERFILES)
...
public class ProfileController {
	...
	@ApiOperation(value = AppPaths.Perfil.ODIN_PERFILES, nickname = "perfiles", notes = "Trae toda la información de perfiles y permisos por perfil de inicio de sesión")
	...
	public ResponseEntity<?> get(HttpServletRequest request,
		...
		try {
			...
			if (null != nombre) {
				...
				logger.info(mrc.getMessage("odin.recurso.GET.name", null, null), "Perfil", nombre);
				...
				if (null != perfil) {
					...
					if (null == id || (perfilExistente != null
						...
						resourceResponse
								.setMessage(mrc.getMessage("odin.security.autorizacion.resourceAccess.OK", null, null));
			...
			if (session != null && session.getUsuario() != null && session.getUsuario().getPerfil() != null) {
				...
				logger.error(mrc.getMessage("odin.security.autorizacion.resourceAccess.session.error", null, null));
			}
		} catch (Exception e) {
			new Exception(e);
			logger.info(mrc.getMessage("odin.recurso.POST.ERROR", null, null), "perfil", e.getMessage());
	...
	public ResponseEntity<?> get(@PathVariable(AppPaths.PATH_VARIABLE_ID) Integer id, HttpServletRequest request) {
		...
		try {
			...
			if (id != null) {
				...
				logger.info(mrc.getMessage("odin.recurso.GET.id", null, null), "Perfil", id);
	...
	public ResponseEntity<?> create(@RequestBody Perfil perfil, HttpServletRequest request) {
		...
		try {
			...
			if (perfil != null) {
				...
				logger.info(mrc.getMessage("odin.recurso.POST", null, null), "Perfil", perfil.getNombre());
				...
				resourceResponse.setMessage(
						mrc.getMessage("odin.security.autorizacion.resourceAccess.POST.success", null, null));
				logger.info(mrc.getMessage("odin.security.autorizacion.resourceAccess.POST.success", null, null));
			...
			logger.error(mrc.getMessage("odin.recurso.POST.ERROR", null, null), "Perfil", perfil.getNombre(),
					e.getMessage());
			...
			if (e.getCause().getCause().getMessage().contains(AppConstants.General.VALIDATE_NOMBRE_UNIQUE)) {
				...
				resourceResponse.setMessage(mrc.getMessage("odin.POST.perfil.business.validation.name", null, null));
		...
		try {
			...
			if (null != id) {
				...
				if (null != perfilFound) {
					...
					logger.info(mrc.getMessage("odin.recurso.PUT", null, null), "Perfil", perfilFound.getId(),
					...
					resourceResponse.setCode(org.apache.http.HttpStatus.SC_OK);
					resourceResponse.setMessage(
							mrc.getMessage("odin.security.autorizacion.resourceAccess.PUT.success", null, null));
			...
			logger.error(mrc.getMessage("odin.security.autorizacion.resourceAccess.PUT.error", null, null),
					e.getMessage());
			new UsuariosException(e);
		}
		resourceResponse.setCode(org.apache.http.HttpStatus.SC_NO_CONTENT);
		resourceResponse.setMessage(mrc.getMessage("odin.security.autorizacion.resourceAccess.NOT_FOUND", null, null));
		logger.error(mrc.getMessage("odin.security.autorizacion.resourceAccess.NOT_FOUND", null, null));
		return ResponseEntity.status(HttpStatus.NO_CONTENT).body(resourceResponse);
	}
	...
	public ResponseEntity<?> delete(
		...
		try {
			...
			if (id != null) {
				...
				if (null != perfilFound) {
					...
					if (perfilSuperior.size() > 0) {
						resourceResponse.setCode(org.apache.http.HttpStatus.SC_PRECONDITION_FAILED);
						resourceResponse.setMessage(mrc.getMessage("odin.PerfilSuperior.DELETE.ERROR", null, null));
						logger.info(mrc.getMessage("odin.PerfilSuperior.DELETE.ERROR", null, null));
						...
						logger.info(mrc.getMessage("odin.recurso.DELETE", null, null), "Perfil", perfilFound.getId(),
								perfilFound.getNombre());
						...
						resourceResponse.setCode(org.apache.http.HttpStatus.SC_OK);
						resourceResponse.setMessage(
								mrc.getMessage("odin.security.autorizacion.resourceAccess.DELETE.success", null, null));
						logger.info(
								mrc.getMessage("odin.security.autorizacion.resourceAccess.DELETE.success", null, null));
			...
			resourceResponse.setMessage(mrc.getMessage("odin.perfil.DELETE.ERROR", null, null));
			resourceResponse.setCode(org.apache.http.HttpStatus.SC_INTERNAL_SERVER_ERROR);
			logger.error(mrc.getMessage("odin.perfil.DELETE.ERROR", null, null), id);
		...
		logger.info(mrc.getMessage("odin.security.autorizacion.resourceAccess.NOT_FOUND", null, null), id);
		return ResponseEntity.status(HttpStatus.NO_CONTENT).body(ResourceResponse.getSingleInstance());
	}

	/**
	 * Genera el reporte de los Perfiles existentes en Aurora. Puede ser 1 o más
	...
	public @ResponseBody void getFileXLSX(HttpServletResponse response,
		...
		try {
			...
			logger.info(mrc.getMessage("odin.security.autorizacion.resourceAccess.reporte.info", null, null),
					" Perfiles - XLSX");

		} catch (Exception e) {
			logger.info(mrc.getMessage("odin.security.autorizacion.resourceAccess.reporte.error", null, null),

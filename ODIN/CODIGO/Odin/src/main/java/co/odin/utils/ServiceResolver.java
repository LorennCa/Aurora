...
public class ServiceResolver {
	...
	public String resolveServiceURL(String uri) {
		...
		if (uri.indexOf(ODIN_INITIAL_CONFIGURATION_CONTEXT) >= 0) {
			...
			sb.append(service.getService_protocol() != null ? service.getService_protocol() : DEFAULT_PROTOCOL)
					...
					.append(service.getService_root_context()).append(ODIN_INITIAL_CONFIGURATION_CONTEXT);
		} else if ((uri.indexOf(GENERAL_ACCESS_CONTEXT) >= 0)) {
			service = ServicesMap.instance().getService("Odin");
			sb.append(service.getService_protocol() != null ? service.getService_protocol() : DEFAULT_PROTOCOL)
					.append(service.getService_host_ip()).append(":").append(service.getService_port())
			...
			else if (uri.indexOf(ODIN_GENERAL_ACCESS_VALIDATE_DATE_CONTEXT) >= 0)
				sb.append(ODIN_GENERAL_ACCESS_VALIDATE_DATE_CONTEXT);
		...
		} else if(uri.indexOf("accesorios")>=0 || uri.indexOf("precompra")>=0 || uri.indexOf("detalles")>=0|| uri.indexOf("carrito")>=0  || uri.indexOf("compras")>=0 || uri.indexOf("pedidos")>=0){
			service = ServicesMap.instance().getService("Aurora");
			sb.append(service.getService_protocol() != null ? service.getService_protocol() : DEFAULT_PROTOCOL).append(service.getService_host_ip()).append(":").append(service.getService_port()).append(uri.replaceAll("odin/", ""));
		}else if(uri.indexOf("productos")>=0 || uri.indexOf("proveedores")>=0 || uri.indexOf("comprasadm")>=0){
			service = ServicesMap.instance().getService("Aquiles");
			sb.append(service.getService_protocol() != null ? service.getService_protocol() : DEFAULT_PROTOCOL).append(service.getService_host_ip()).append(":").append(service.getService_port()).append(uri.replaceAll("/Aurora/odin/", "/Aquiles/"));
		}

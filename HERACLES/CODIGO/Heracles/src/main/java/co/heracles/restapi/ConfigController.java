...
import co.common.response.AppConfResponse;
import co.heracles.config.ConfigurationHeracles;
import co.heracles.config.appconfig.AppInitParams;
import co.heracles.entities.ConfiguracionHeracles;
import co.heracles.utils.AppConstants;
...
public class ConfigController {
	...
	@ApiResponses(value = {
			@ApiResponse(code = org.apache.http.HttpStatus.SC_OK, message = "<cadena toString del objeto de respuesta>",
						 response = ConfiguracionHeracles.class),
	...
	public ResponseEntity<?> getSEEConfig(){
		...
		return ResponseEntity.status(HttpStatus.OK).body(ConfigurationHeracles.instance().getConfig());
	}

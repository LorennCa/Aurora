angular.module('aurora.proveedoresController', ['ngIdle', 'ngMaterial'])

    .controller('proveedoresController', function ($scope, $route, $state, $mdEditDialog, $mdMedia, $mdToast, $location, $mdSidenav, $mdDialog, $sce, mainService, configurationServer, globalConstant, localStorageService, resourceAccessService, loginService, userService) {
    ...
    $scope.proveedores = function(ev){
        $scope.progressBar = true;
        console.log("Proveedores");
        $scope.referencia = $scope.referenciaBuscada;
        console.log($scope.referencia);
            resourceAccessService.nameValidate($scope.userScope,$scope.userToken, '/proveedores/consultar',$scope.referencia,null).then(function onSuccess(data) {
                if (data.status == 200) {
                    //actualizar token 
                    responseOk(data);
                    //console.log('Lista: ' + JSON.stringify(data.data.otrasOpciones.listaInventario));
                    if (data.data.otrasOpciones.proveedor.length > 0) {
                        $scope.proveedores = data.data.otrasOpciones.proveedor;
                        $scope.proveedorLen = $scope.proveedores.length;
                        $scope.referenciaBuscada = "";
                        $scope.progressBar = false;
                    } else {
                        $scope.progressBar = false;

angular.module('aurora.inventarioController', ['ngIdle', 'ngMaterial'])
    ...
    $scope.inventarioDisp = function(ev){
        ...
        console.log($scope.referencia);
            resourceAccessService.nameValidate($scope.userScope,$scope.userToken, '/productos/inventario',$scope.referencia,null).then(function onSuccess(data) {
                if (data.status == 200) {
                    //actualizar token 
                    responseOk(data);

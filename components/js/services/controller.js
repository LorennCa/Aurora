...
angular.module('aurora.controller', ['ngIdle', 'ngMaterial'])
    ...
    if ($scope.generalOpc != null) {
        ...
        function validarAcceso(resource) {
            if (resource.includes('productos')||resource.includes('accesorios')||resource.includes('precompra') ||resource.includes('proveedores')||resource.includes('pedidos')||resource.includes('compras')||resource.includes('detalles')||resource.includes('carrito')){
                ...
                resourceAccessService.validarAcceso($scope.userToken, $scope.userScope, resource).then(function onSuccess(data) {
                    if (data.status == 200) {
                        ...
                        if ((!resource.includes('inventario'))&&(!resource.includes('buscar'))) {
                            ...
                            //console.log("es aurora-->>" + resource + "-->>" + resource.substring(1, resource.length).replace(new //RegExp('-', 'g'), ''));

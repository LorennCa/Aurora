angular.module('aurora.detallesController', ['ngIdle', 'ngMaterial'])

    .controller('detallesController', function ($scope, $route, $state, $mdEditDialog, $mdMedia, $mdToast, $location, $mdSidenav, $mdDialog, $sce, mainService, configurationServer, globalConstant, localStorageService, resourceAccessService, loginService, userService) {
    ...
    if ($scope.generalOpc != null) {
        ...
        *************************************/
        ...
        //    console.log('retrocedemos una pagian')
        //    function enableBack() {
        //        window.history.back();
        //    }
        //});
    ...
    $scope.mostrarDetalleProducto = function(){
        $scope.cargarDetalles = localStorageService.get('detalleProducto');
        console.log("hoerf")
        console.log($scope.cargarDetalles);
    }
    
    $scope.carrito = function(ev){
            $scope.progressBar = true;
        console.log("InventarioDispo");
            resourceAccessService.validarAcceso($scope.userToken, $scope.userScope, '/precompra/consultar').then(function onSuccess(data) {
                if (data.status == 200) {
                    //actualizar token 
                    responseOk(data);
    ...
    function armarJsonNuevo() {
        ...
        console.log($scope.producto);
            $scope.dataReferencia = {
                ...
                recurso:$scope.producto.recursoUno,
